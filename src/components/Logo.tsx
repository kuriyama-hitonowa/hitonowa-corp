...
export function Logo({
  ...
  const sizeClasses = {
    sm: { text: "text-xl tracking-tight", sub: "text-[9px] tracking-[0.24em]", gap: "gap-0.5" },
    md: {
  ...
  return (
    <div className={`inline-flex flex-col items-center select-none ${className}`}>
      <div className={`font-bold ${current.text} flex items-baseline leading-none`}>
        <span className="text-ink font-semibold tracking-[-0.02em]">{site.brand.logoLead}</span>
        <span className="text-[#EFA00B] font-bold tracking-[-0.01em]">{site.brand.logoTail}</span>
      </div>
      {showSubtitle && (
        <div
